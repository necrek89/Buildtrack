...
export function Dashboard() {
  ...
  const createProject = async () => {
    ...
    if (!error) {
      fetchProjects()
      setShowAdd(false)
      setForm({ name: '', stage: 'Foundation', deadline: '', address: '' })
    }
  }
            ...
            {isOpen && (
              <div style={{ borderTop:'1px solid #EAE3D8', padding:'12px 13px', background:'#FDFBF8' }}>
                ...
                <div
                  ...
                  style={{ fontSize:12, fontWeight:600, color: p.address ? '#C96B3A' : '#B8AFA6', cursor: p.address ? 'pointer' : 'default', textDecoration: p.address ? 'underline' : 'none', marginBottom:3 }}
                >
                  {p.address || '— address not set'}
                </div>
                <div style={{ fontSize:11, color:'#B8AFA6', marginBottom:10 }}>
                  Due {p.deadline || 'not set'}
                  {p.deadline && <span style={{ color:'#C96B3A', fontWeight:600, marginLeft:6 }}>· {Math.max(0, Math.ceil((new Date(p.deadline) - new Date()) / 86400000))} days left</span>}
                </div>
                <div style={{ height:1, background:'#EAE3D8', marginBottom:10 }} />
                <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:6, marginBottom:10 }}>
                  {[{v:pPct+'%',l:'Progress'},{v:pActive,l:'Active',c:'#C96B3A'},{v:pDone,l:'Done',c:'#5A9467'}].map(s => (
